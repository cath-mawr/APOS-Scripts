...
public final class S_TreeFletch extends Script
	...
	static class TreeType {
		...
		TreeType(String name, int[] ids_trees, int id_log, int lvl,
		    ItemType... items) {
	...
	public int main() {
		...
		if (mode != MODE_FLETCH && (System.currentTimeMillis() >= (last_cut + wb_max_time) || distanceTo(start_x, start_y) >= max_range)) {
			if (!pw_init) {
				pw.init(null);
				pw_init = true;
			}
