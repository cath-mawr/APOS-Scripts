...
public final class S_KBDLooter extends Script
	...
	public void paint()
	{
		...
		if (autobury.getState()) {
			...
			drawString("Prayer XP: " + int_format(gained) +
			    " (" + per_hour(gained) + "/h)", x, y, font, white);
			y += 15;
		}
		x += 10;
		...
		for (int i = 0; i < loot.length; ++i) {
			...
			drawString(banked_counts[i] + " " + getItemNameId(loot[i]) +
			    " (" + per_hour(banked_counts[i]) + "/h)", x, y, font, white);
			y += 15;
		}
	}
	...
	public void onServerMessage(String str)
	{
		...
		if (str.contains("busy")) {
			...
			System.out.println("Spider room (expected): " + in_spider_area() +
			    " | kbd room (unexpected): " + in_fight_area());
	...
	private void print_out()
	{
		...
		if (autobury.getState()) {
			System.out.println("Prayer XP gained: " +
			    int_format(pray_xp - start_pray_xp));
		}
		for (int i = 0; i < loot.length; ++i) {
			if (banked_counts[i] <= 0) continue;
			System.out.println("Banked " +
			    banked_counts[i] + " " + getItemNameId(loot[i]) +
	...
	public void actionPerformed(ActionEvent e)
	{
		if (e.getActionCommand().equals("OK")) {
			...
			pw.init(null);
			lumb_to_dray = pw.calcPath(LUMB_X, LUMB_Y,
			    DRAY_BANK_X, DRAY_BANK_Y);
			edge_through_wild = pw.calcPath(EDGE_BANK_X, EDGE_BANK_Y,
			    KBD_NEAR_GATE_X, KBD_NEAR_GATE_Y);
